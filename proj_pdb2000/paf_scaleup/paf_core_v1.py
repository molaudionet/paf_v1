...
def ligand_centroid(chain, ligand_resname: Optional[str], params: PAFParams) -> Optional[np.ndarray]:
    ...
    for res in chain.get_residues():
        ...
        if resname in _LIGAND_BLACKLIST:
            # If user explicitly requested this resname, don't blacklist it
            if not (ligand_resname and resname == ligand_resname):
                continue
...
def kinase_center_and_anchor2(chain, ligand_resname: Optional[str], params: PAFParams) -> Tuple[np.ndarray, np.ndarray, Dict[str, str]]:
    ...
    # Motif fallback
    if motifs is None:
        # Last-resort fallback: geometric center of all CA atoms in chain
        all_ca = []
        for res in chain.get_residues():
            ...
            if hetflag != " ":
                continue
            if "CA" in res:
                all_ca.append(np.array(res["CA"].get_coord(), dtype=np.float32))
        if not all_ca:
            raise RuntimeError("No ligand centroid, no kinase motifs, and no CA atoms in chain.")
        center = np.mean(np.stack(all_ca), axis=0)
        meta = {
            "center_method": "chain_geometric_center",
            "anchor2_method": "center_fallback",
            "warning": "No ligand centroid found and kinase motifs not detected; "
                       "using geometric center of all CA atoms as fallback."
        }
        return center, center, meta
...
def embed_pocket(
    ...
    meta = {
        ...
        "center_mode": pocket.meta.get("center_method","unknown"),
